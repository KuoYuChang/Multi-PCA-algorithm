...
# reference: 
# https://github.com/whuirlab/Spectral-clustering-HW
...
#                self.D[i,i] += 1
    ...
    # self add
    ## construct W, D, turn self.graph = True
    def input_graph(self, W):
        #----------------------
        # input similarity matrix if already have
...
#                self.w, self.U = eigsh(self.L, k, self.D) #using method for sparse matrices
    ...
    #---------------------------------------------------------------------------------
    # for orginal(arthor) version random walk, seems like the ability of generalized eigen problem for scipy..eigh not approximate enough 
    ## self try
    """Cluster the objects via normalized spectral clustering (L_rw) into k clusters"""
    def norm_rw_sc_mod(self, k):
        if self.graph == False: print("Construct a similarity graph first!")
        else:
            self.L = self.D - self.W #compute the unnormalized Laplacian
#            if self.graph == "fully connected graph":
            m = self.W.shape[0]
            D_inv = np.zeros([m,m])
            D_inv[np.diag_indices(m)] = 1/np.sum(self.W, 1)
            #print(D_inv)
            self.L = np.matmul(D_inv,self.L)
            self.w, self.U = eigh(self.L, eigvals=(0,k-1)) #generalized eigenproblem L u = lambda D u
#            else:
#                self.w, self.U = eigsh(self.L, k, self.D) #using method for sparse matrices
            kmeans = KMeans(k)
            self.pred = kmeans.fit_predict(self.U)
        self.clustering = "normalized (rw) Laplacian"
    #
    # seems like directly compute eigenvector of D_inv L is ok
    # why generalized problem failed?
    #--------------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------------------------------------------------
# my function
# to get spectral clustering result in a function, similar as sklearn

def spec_get_L(A, NOR_RAN=True):
    # initial
    spec = spectral(None, None, None, None)
    spec.input_graph(A)
    
    # normalized random walk
    # for conveient to get Laplacian matrix, k random set
    if NOR_RAN:
        print('In Whuir_spectral.py, using normalized random walk!')
        spec.norm_rw_sc_mod(k=2)# using our modified, to prevent rounding error for generalized eigenvector compute
    # else do unnormalized Laplacian
    else:
        spec.unnorm_sc(k=2)
    L = spec.L
...
def spec_clus_Lrw(A, k_clus, NOR_RAN=True):
    '''
    parameters
    ----------
    
    A: an weight matrix that every element is non-negative, shape: (n_samples, n_samples)
    
    k_clus: number of clusters
    
    NOR_RAN: choose normalized random walk Laplacian if True, else choose unnormalized Laplacian
    
    
    Returns
    -------
    labels: array of integers, the labels of the clusters
    '''
    ...
    if NOR_RAN:
        print('In Whuir_spectral.py, using normalized random walk!')
        spec.norm_rw_sc_mod(k=k_clus)# using our modified, to prevent rounding error for generalized eigenvector compute
    # else do unnormalized Laplacian
    else:
        spec.unnorm_sc(k=k_clus)
    labels = spec.pred
